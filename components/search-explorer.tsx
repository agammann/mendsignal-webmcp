...
export function SearchExplorer({ initialQuery = '' }: { initialQuery?: string }) {
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      setResults(localResults.filter((item) => {
  ...
  return (
    <div className="explorer-layout">
      <aside className="filter-panel">
        ...
        <button className="secondary-button" onClick={runSearch}><SlidersHorizontal /> Apply filters</button>
      ...
      <section className="results-panel">
        ...
        {!loading && !results.length && <div className="empty-state"><Search /><h2>No matching repair evidence yet</h2><p>Start a case and help create the first verified trail for this problem.</p><a href="/repair/new">Start this repair</a></div>}
      </section>
    </div>
  );
}
