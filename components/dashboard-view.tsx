...
export function DashboardView() {
  ...
  return <div className="dashboard-shell">
    ...
    <div className="dashboard-grid"><section className="category-chart"><div className="section-heading"><div><p className="mono-label">MOST REPAIRED</p><h2>Categories in the memory</h2></div></div><div className="category-bars">{stats.most_repaired_categories.map((item: any) => <div key={item.category}><span>{item.category}<b>{item.count}</b></span><div><i style={{ width: `${item.count / maxCategory * 100}%` }} /></div></div>)}</div></section><section className="dashboard-callout"><p className="mono-label">WHY IT COMPOUNDS</p><h2>Every outcome makes the next search better.</h2><p>Pulse records failed attempts alongside successes, so agents can compare evidence instead of repeating the most popular advice.</p><a href="/repairs">Explore repair evidence <ArrowUpRight /></a></section></div>
